...
class Buyer {
	...
	void buy(Product p) {
		/*
		 * 1.1 가진 돈과 물건의 가격을 비교해서 가진 돈이 적으면 메서드를 종료한다. 1.2 가진 돈이 충분하면, 제품의 가격을 가진 돈에서 빼고
	...
	void add(Product p) {
		 ...
		 * 장바구니의 내용을 새로운 배열에 복사한다. 1.1.3 새로운 장바구니와 기존의 장바구니를바꾼다. 1.2 물건을 장바구니(cart)에
		 * 저장한다. 그리고 i의 값을 1 증가시킨다.
		 */
		if (i >= cart.length) {
	...
	void summary() {
		/*
		 * 1.1 장바구니에 담긴 물건들의 목록을 만들어 출력한다. 1.2 장바구니에 담긴 물건들의 가격을 모두 더해서 출력한다. 1.3 물건을 사고
		 * 남은 금액(money)를 출력한다.
		 */
		String pnames = "";
		int totamt = 0;
